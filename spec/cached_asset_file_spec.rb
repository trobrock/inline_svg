...
describe InlineSvg::CachedAssetFile do
  ...
  it 'differentiates two files with the same name' do
    known_document0 = File.read(fixture_path.join('assets0', 'known-document.svg'))
    known_document1 = File.read(fixture_path.join('assets1', 'known-document.svg'))

    expect(known_document0).not_to eq(known_document1)

    asset_loader = InlineSvg::CachedAssetFile.new(paths: fixture_path)

    expect(known_document0).to eq(asset_loader.named('assets0/known-document.svg'))
    expect(known_document1).to eq(asset_loader.named('assets1/known-document.svg'))
  end

  it 'chooses the closest exact matching file when similar files exist in the same path' do
    known_document = File.read(fixture_path.join('assets0', 'known-document.svg'))
    known_document2 = File.read(fixture_path.join('assets0', 'known-document-two.svg'))

    expect(known_document).not_to eq(known_document2)
    ...
    expect(asset_loader.named('known-document-two')).to eq(known_document2)
  end

  it 'filters wanted files by simple string matching' do
    File.read(fixture_path.join('assets0', 'known-document.svg'))
    known_document1 = File.read(fixture_path.join('assets1', 'known-document.svg'))
